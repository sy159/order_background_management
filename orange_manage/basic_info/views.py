...
def edit_accountinfo(request):
    if request.method=='GET':
        ...
        if get_level==0:
            ...
            get_region = obj.open_admin_region
        if get_pwd:
            models.Admin.objects.filter(account=get_account).update(pwd=get_pwd,realname=get_realname,phone=get_phone,email=get_email,qq=get_qq,status=get_status,level=get_level,open_admin_region=get_region)
        else:
            models.Admin.objects.filter(account=get_account).update(realname=get_realname,phone=get_phone,email=get_email,qq=get_qq,status=get_status,level=get_level,open_admin_region=get_region)
